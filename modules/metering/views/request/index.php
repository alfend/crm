...
<div class="request-index">
    ...
    foreach ($array_request as $request){
        ...
        if(!$response->cheсkResponse($request['id'],Yii::$app->user->getId(),User::TYPE_METERING)) {
            $button_res=Html::a('Откликнуться', ['/metering/default/create-response/'],['data-method' => 'POST', 'data-params' => ['id_request' => $request['id'], 'id_workers' => Yii::$app->user->getId(), 'type_workers' => User::TYPE_METERING]], ['class' => 'btn btn-primary']);
        } else {
//            $button_res='Вы уже откликнулись';
